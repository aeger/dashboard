      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setShowEditor(true)} className="text-xs text-zinc-500 hover:text-zinc-300 transition-colors">
              edit feeds
            </button>
            <button onClick={load} disabled={loading} className="text-xs text-zinc-500 hover:text-zinc-300 transition-colors disabled:opacity-40">
              refresh
            </button>
          </div>
        </div>
                ...
                <li key={i} className={`rounded-lg border transition-all ${isExpanded ? 'border-zinc-700 bg-zinc-800/50' : 'border-transparent hover:border-zinc-800 hover:bg-zinc-800/20'}`}>
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="text-sm text-zinc-300 leading-snug line-clamp-2">{item.title}</div>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-xs text-zinc-600">{item.source.replace('r/', '')}</span>
                        <span className="text-[10px] text-zinc-700">{formatAge(item.pubDate)}</span>
                      </div>
                    </div>
